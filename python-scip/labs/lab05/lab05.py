...
def my_reduce(combiner, seq):
    ...
    if len(seq) == 1:
        return seq[0]
    init = combiner(seq[0], seq[1])
    for i in range(2, len(seq)):
        init = combiner(init, seq[i])
    return init
...
def distance(city_a, city_b):
    ...
    5.0
    """
    x_a, x_b = get_lat(city_a), get_lat(city_b)
    y_a, y_b = get_lon(city_a), get_lon(city_b)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a = distance(des_city, city_a)
    dis_b = distance(des_city, city_b)
